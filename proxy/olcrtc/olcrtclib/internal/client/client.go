...
func (c *Client) startControlLoop(
	...
	// packets under load. Transports sharing one plane keep the conservative
	// default so a dead link is detected promptly. A user-set
...
func (c *Client) sendConnectRequest(stream *smux.Stream, targetAddr string, targetPort int) error {
	...
	// get a generous deadline. The others use the conservative window so a
	// stuck CONNECT fails fast.
